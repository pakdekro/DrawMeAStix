...
- **Four frameworks, one case.** ATT&CK for the intrusion (its three matrices, Enterprise, Mobile and ICS, searched as one corpus), MITRE F3 for the fraud, side by side in the same panel and not in separate modes. F3 reuses 43 ATT&CK techniques by number and six of its eight tactics, so a technique picked on either side is one object in the bundle: an actor can be shown phishing its way in and cashing out, in a single graph that a CERT reads from the left and a fraud team from the right. MITRE ATLAS sits beside them for attacks on AI systems and AADAPT for digital asset payments, neither borrowing an identifier: an intrusion that ends in a poisoned model, or a fraud that cashes out on a chain, is still one graph. All three are searched the same way, in one panel with one switch.
