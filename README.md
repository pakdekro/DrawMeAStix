...
Then point the app at it in Settings. **With no endpoint configured, the application cannot make a network call at all** — that is enforced by the code, not by policy.

## Design principles

1. **Local-first, TLP-safe by construction.** The whole application runs in the browser: STIX logic in TypeScript, storage in IndexedDB. The server only ever serves static files — your TLP:AMBER and TLP:RED drafts never touch it. One public instance can serve several CERTs without a single byte of their data transiting.
...
- **deterministic identifiers** (UUIDv5 over the distinguishing properties, OpenCTI's algorithm, locked down by golden vectors taken from `pycti`). The spec *recommends* UUIDv4 for SDOs but mandates nothing: a third-party tool takes these identifiers like any others. The benefit is universal — two exports of the same content yield the same identifier, so no duplicates — and it is greatest on platforms that deduplicate on them;
