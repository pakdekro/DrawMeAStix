...
- **Three frameworks, one case.** ATT&CK for the intrusion (its three matrices, Enterprise, Mobile and ICS, searched as one corpus), MITRE F3 for the fraud, side by side in the same panel and not in separate modes. F3 reuses 43 ATT&CK techniques by number and six of its eight tactics, so a technique picked on either side is one object in the bundle: an actor can be shown phishing its way in and cashing out, in a single graph that a CERT reads from the left and a fraud team from the right. MITRE ATLAS sits beside them for attacks on AI systems, and borrows nothing at all: its numbers are its own, so an intrusion that ends in a poisoned model is still one graph. All three are searched the same way, in one panel with one switch.
...
- **A STIX guide** at `#/guide`, built from the relationship matrix itself, for whoever has never touched the format, and a page for each framework: `#/attack`, `#/f3` and `#/atlas`, for how a matrix is read and what one of its numbers becomes here. All four are also served as plain HTML at `/guide`, `/attack`, `/f3` and `/atlas`, readable without JavaScript.
