 ...
 * direction: 44 of its techniques adapt an ATT&CK one and 42 carry a name
...
describe("the distilled dataset", () => {
  ...
  it("borrows no ATT&CK identifier, in either direction", () => {
    ...
    expect(adapting).toHaveLength(44);
    for (const e of adapting) expect(e.attack).toMatch(/^T\d{4}(\.\d{3})?$/);
  });

  /**
   * Forty-two names exist in both catalogues. That is not a collision to
