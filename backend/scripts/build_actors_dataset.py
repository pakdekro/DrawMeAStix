...
THE GALAXY'S VERSION IS NOT A SNAPSHOT IDENTIFIER: v350 gained four actors
without moving. The version stays in the file, said as what it is rather than
as a fingerprint, and the digest of the bytes actually read goes to the build
report, where the pull request and the run log keep it. That digest
deliberately does NOT go into the file: it would move whenever the galaxy
touches an actor we drop, and the weekly job would open a pull request whose
only change is a hash. Merging those on sight is the habit this job exists to
prevent.
...
def fetch() -> tuple[dict, str]:
    """The galaxy, and the digest of the bytes it arrived in."""
...
def main() -> None:
    ...
    payload = {
        ...
        "source": (
            f"MISP galaxy threat-actor v{galaxy.get('version')} (CC0)"
            "; its version does not move with every change"
        ),
    ...
    print(f"  read from the galaxy at sha256 {digest}")
